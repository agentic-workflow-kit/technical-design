...
`technical-design` is a self-contained set of AI skills for the design stage: **frame** a
problem, **author** a DDD-first design, **review** it in a decision-recording loop, **enforce**
its boundaries in CI, and **orchestrate** the flow. Methodology-neutral shell; DDD-first in v1
(profile under `methodologies/ddd/`). It owns one contract downstream tools build on: the
...
Source lives outside `docs/`: `skills/`, `methodologies/`, `evals/`, `scripts/`. The five skills
are stable; methodology-specific behavior lives under `methodologies/`.

## Gate and conventions

- **`pnpm check`** before claiming any change done; show its output as evidence. It runs
  prettier, then `skills:check` (skill/profile static checks), then `eval:enforce` (the
  dependency-cruiser eval). The enforce eval **requires seeded violations** so the gate cannot
